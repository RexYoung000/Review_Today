...
> 状态：`mascot-bangs-ahoge-study-v4.png` 已被 Rex 接受为当前角色母形方向；`mascot-card-expression-study-v7.png` 左侧已被接受为当前静态品牌构图与默认表情方向；V10 拆件视觉已被否决，V11 只通过角色完整性、未通过说话表现；V12 已完成实现与原生冒烟，等待代表速度录屏和 Rex 最终体验验收

## V12 真实说话表现（实现与冒烟通过，待体验验收）

- Rex 确认 V11 说话状态过于接近待机：嘴型没有跟随语音，整体表现简陋，不能视为之前讨论的 Spine 效果。
- V12 保留完整、连体的语音角色，只增加 GPT Image 生成的无嘴母图及局部嘴型／闭眼覆盖层；不重新生成或拆分头、身体、手臂和耳机。
- 本机真实 TTS 播放音量驱动闭口、小开口、中开口和大开口；方牙笑只在句末短暂出现。眨眼、呆毛和身体重心采用独立低幅节奏，不让全身机械跟随音量。
- V12 第一张五档嘴型表被 Rex 发现存在统一平直上沿，连续排列时形成明显横向截断带，视觉上像贴纸或裁切错误。该嘴型表已作废；替换稿改为完整、微不规则的封闭椭圆轮廓，上下都有弧度，且所有不透明像素与素材边缘保持安全距离。
- 替换稿、五状态预览与原生高音量运行截图分别为 `mascot-mouth-sheet-v12.png`、`mascot-mouth-animation-preview-v12.png` 和 `mascot-tts-speaking-wide-v12.png`。真实 TTS 已驱动到大开口，原生画面未再出现横向截断；完整录屏仍未完成。
- V12 的通过证据必须是代表速度的真实 Mac 录屏，覆盖连续说话、停顿、句末、打断和 Reduce Motion；静态截图、构建与手动滑杆只作辅助证据。
...
- Rex 已确认 V8.3 人物显示范围可以锁定。V8.4 已从同一完整吉祥物与单色标志母图得到真实 Alpha 资产，不改造型、不重新生成构图；暖白底母图继续保留为回退。
- 代码只能按已接受草图的取样中位色移动模型已有像素，以及对 App 图标母图进行裁切、缩放、圆角 Alpha 蒙版和 PNG 导出；不能补画五官、手臂、卡片、刘海或呆毛，也不能用 SVG 重新解释模型线条。
- 内置生成工具对完整吉祥物的两次去底都把透明棋盘格画进 RGB 文件，因此这两次结果被拒绝；单色标志的模型提取成功输出真实 Alpha。透明吉祥物改为从已接受 GPT Image 母图确定性移除连通背景，并只对外沿去除暖白污染，不新增或重画角色像素。两项 cutout 均已通过 `hasAlpha: yes` 与暖白／陶土叠底检查，仍等待 Rex 视觉验收；深色叠底只用于暴露边缘风险，不代表深色版本已完成。
