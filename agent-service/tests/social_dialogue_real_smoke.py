...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='review-social-dialogue-') as directory:
        ...
        with RunReport(output, layer='live_generated_flow', planned=[f'turn-{i:02}' for i in range(1, 20)],
            ...
            def send(sid, text, *, kind=None, preserve=False, no_task=True):
                ...
                replies = [m['content'] for m in data['messages'] if m['role']=='coach' and m['run_id']==run['run_id']]
