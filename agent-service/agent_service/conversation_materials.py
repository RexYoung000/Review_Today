...
class MaterialReadiness(BaseModel):
    ...
    def validate_request(self, payload):
        ...
        if self.can_proceed and not any(f.sufficient and (payload['kind'] != 'jd' or f.role == 'jd') for f in self.findings):
...
def normalize(data, decision, last):
    ...
    continuing = bool(task and task['context'].get('material_kind') == 'jd'
                      ...
                      and ('material' in decision.intents or task['context'].get('awaiting_material')
                           and set(decision.intents) <= {'continue', 'confirm', 'goal'}))
...
def prepare(h, sid, rid, rev, decision, reader):
    ...
    needs_check = decision.is_jd or bool(urls) or bool(prior.get('awaiting_material')
    ...
    if needs_check:
        assessment = h._call(sid, rid, rev, 'material_readiness', READINESS,
            json.dumps(dict(kind='jd' if decision.is_jd else 'source', context=context,
                            sources=answer_sources(sources), reads=states,
                            original_request=prior.get('material_goal', last['content'])), ensure_ascii=False), MaterialReadiness, ROUTER_MODEL)
    with h.store.transaction(sid, rid, rev) as current:
        ...
        if assessment:
            ...
            if not assessment.can_proceed and live_task and pending.get('kind') == 'select_question' and pending.get('target_id') == live_task['task_id']:
    ...
    if assessment and not assessment.can_proceed:
        h._publish(sid, rid, rev, assessment.reply, stage='awaiting_material' if task else None,
                   required={'type': 'respond', 'prompt': '补充材料正文', 'options': []} if task else None)
