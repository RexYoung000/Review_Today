...
enum ConversationProcessor {
    ...
    static func tick(context: ModelContext, monitor: AgentServiceMonitor) async {
        ...
        let controls = (try? context.fetch(FetchDescriptor<AgentRunControl>(sortBy: [SortDescriptor(\.createdAt)]))) ?? []
        ...
        let messages = (try? context.fetch(FetchDescriptor<AgentMessage>(sortBy: [SortDescriptor(\.createdAt)]))) ?? []
        for message in messages where message.role == "user" && message.deliveryStatus == "local" {
            ...
            do {
                let recent = messages.filter { $0.sessionID == session.id && $0.id != message.id && $0.createdAt < message.createdAt }
                    .suffix(10).map { ["role": $0.role == "assistant" ? "coach" : $0.role, "content": String($0.content.prefix(3000))] }
                ...
                if !runs.contains(where: { $0.id == id }) { context.insert(run) }
                run.status = message.deliveryStatus
                try context.save()
            } catch {
    ...
    @MainActor
    private static func persist(_ page: [String: Any], session: AgentSession, context: ModelContext) throws {
        let existingEvents = (try context.fetch(FetchDescriptor<SessionEventRecord>()))
        var existingMessages = Set(try context.fetch(FetchDescriptor<AgentMessage>()).map(\.id))
        var tasks = try context.fetch(FetchDescriptor<LearningTask>())
        var runs = try context.fetch(FetchDescriptor<AgentRun>())
        ...
        for raw in events {
            ...
            if existingEvents.contains(where: { $0.id == id }) { continue }
            ...
            event.payloadJSON = json(raw) // full audit and structured references, no hidden reasoning
            event.occurredAt = date(raw["occurred_at"])
            context.insert(event)
            if let message = raw["message"] as? [String: Any], let messageID = uuid(message["message_id"]), !existingMessages.contains(messageID) {
                let saved = AgentMessage(id: messageID, sessionID: session.id, taskID: uuid(raw["task_id"]), role: "assistant",
                                         content: message["content"] as? String ?? "", createdAt: date(message["created_at"]), deliveryStatus: "received")
                saved.runID = runID
                context.insert(saved)
                existingMessages.insert(messageID)
            }
            ...
            if let taskJSON = payload["task"] as? [String: Any], let taskID = uuid(taskJSON["task_id"]),
        ...
        let pendingControls = try context.fetch(FetchDescriptor<AgentRunControl>()).filter { $0.sessionID == session.id && !$0.sent }
