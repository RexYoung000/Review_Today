...
import {createHeaderPlayer} from '@review-motion/header-player.mjs';
...
function poseRecall(dt){
 ...
 if(config.header){if(config.reduced)header.reset();else header.advance(dt);header.apply(s);}
 else if(config.reduced){time=0;returnState=null;}
...
function paint(dt){
 ...
 if(config.surface==='voice'){
   ...
   poseRecall(dt);ctx.clearRect(0,0,width,height);ctx.save();const view=config.header?{x:width/2,y:height*.49,scale:Math.min(width/300,height/270)}:config.ambient?idleFraming(width,height):{x:width/2,y:height*.49,scale:Math.min(width/410,height/340)};ctx.translate(view.x,view.y);ctx.scale(view.scale,-view.scale);renderer.draw(rig.skeleton);ctx.restore();
 }
 frames++;
}
...
window.mascotMotion={
 headerInput(input){
   if(!config.header||!config.visible||config.reduced)return;
   if(input.type==='pointer')header.pointer(input.x,input.y);
   else if(input.type==='poke')header.poke();
   else if(input.type==='reset')header.reset();
   else header.quiet();
   wake();
 },
 setState(next){
