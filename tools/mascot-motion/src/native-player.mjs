...
let config={surface:'recall',mode:'idle',level:0,reduced:false,dark:false,visible:true,rate:1},rig,voice,raf=0,last=0,time=0,returnState,returnElapsed=0,frames=0;
...
function paint(dt){
 ...
 if(config.surface==='voice'){
   ...
   drawVoiceScene(ctx,renderer,rig,voice,width,height,{dark:config.dark,compact:width<300});
 }else{
...
function wake(){if(!raf&&rig)raf=requestAnimationFrame(frame);}
window.mascotMotion={
 setState(next){
   const previous=config;
   config={surface:next.surface==='voice'?'voice':'recall',mode:['listening','thinking','speaking','idle'].includes(next.mode)?next.mode:'idle',level:Math.max(0,Math.min(1,Number(next.level)||0)),reduced:!!next.reduced,dark:!!next.dark,visible:!!next.visible,rate:next.rate===.5?.5:1};
 ...
 inspect(){return {ready:!!rig,frames,animating:!!raf,config,time,voice:voice?{height:voice.contact.height,phase:voice.contact.phase,area:voice.contact.soft.areaRatio,contacts:voice.contact.soft.contacts.length}:null};}
};
...
try{
 ...
 const data=new spine.SkeletonJson(new spine.AtlasAttachmentLoader(atlas)).readSkeletonData(asset.json);rig={data,skeleton:new spine.Skeleton(data)};voice=createVoiceState(contactGeometry(spine,rig));paint(0);send('ready');wake();
