...
let rig,materials,raf=0,last=0,elapsed=0,clipTime=0,clip='',wait=0,done=false,frames=0,sequence=createSequence(),returnPose=null,runtimeError='';
...
function duration(){return durations[clip]??rig.data.findAnimation(clip)?.duration??1;}
function apply(dt){
...
function frame(now){raf=0;if(!rig)return;const dt=Math.min(.05,(now-(last||now))/1000);last=now;try{paint(config.visible?dt:0);}catch(error){runtimeError=String(error);done=true;send('failed',{message:runtimeError});return;}if(config.visible&&!config.reduced&&!done&&config.kind!=='rest')raf=requestAnimationFrame(frame);else last=0;}
...
window.mrB={setState(next){
 ...
 if(old.visible!==config.visible||old.reduced!==config.reduced)last=0;wake();
},inspect(){return {runtimeError,ready:!!rig,frames,animating:!!raf,clip,clipTime,done,config};}};
