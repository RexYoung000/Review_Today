import {reactionFrame,returnReaction,reactionDuration} from '@review-motion/reaction-scene.mjs';
...
import {reviewFrame,reviewDuration} from '@review-motion/settlement-sequences.mjs';
...
let flow=createSettlementFlow(),lastFlowPhase='',flowCompletionSent=false,configured=false;
let reactionSource=null;
const isReaction=()=>config.kind.startsWith('reaction_');
const currentReaction=()=>config.kind==='reaction_rest'?returnReaction(reactionSource,clipTime,config.reduced):reactionFrame(config.kind,clipTime,config.reduced);
const isStudy=()=>isReaction()||config.kind==='flow_study'||config.kind==='review_study'||!!studyDuration[config.kind];
...
function reset(){elapsed=clipTime=0;done=false;sequence=createSequence();flow=createSettlementFlow({compact:config.compact});flowCompletionSent=false;lastFlowPhase='';lastTick=-1;clip=config.kind==='thinking'?'mr_receive':config.kind==='idle'?'idle_look':config.kind;wait=1;returnPose=null;}
function duration(){if(isReaction())return config.kind==='reaction_rest'?.18:reactionDuration;return config.kind==='flow_study'?flow.duration():config.kind==='review_study'?reviewDuration:studyDuration[clip]??durations[clip]??rig.data.findAnimation(clip)?.duration??1;}
function apply(dt){
 if(isReaction()){if(config.reduced){clipTime=duration();finish();}else if(!config.paused&&!done){clipTime=Math.min(duration(),clipTime+dt);if(clipTime>=duration())finish();}return;}
 ...
 if(config.kind==='review_study'&&config.reduced){clipTime=duration();finish();return;}
 if(config.kind==='review_study'||studyDuration[config.kind]){if(!config.paused&&!done&&!config.reduced){clipTime=Math.min(duration(),clipTime+dt);if(clipTime>=duration())finish();}return;}
...
function drawScene(w,h){
 if(isReaction()){studyInfo=studyPlayer.draw(config.kind,clipTime,w,h,config,currentReaction());if(Math.abs(clipTime-lastTick)>=.09||done){lastTick=clipTime;send('studyTime',{time:clipTime});}return;}

 if(config.kind==='review_study'){
  const t=config.reduced?reviewDuration:clipTime;studyInfo=studyPlayer.draw(config.kind,t,w,h,config,reviewFrame(t));
  if(Math.abs(t-lastTick)>=.09||done){lastTick=t;send('studyTime',{time:t});}return;
 }
 if(config.kind==='flow_study'){
...
function wake(){if(configured&&rig&&!raf)raf=requestAnimationFrame(frame);}
window.mrB={setState(next){
 configured=true;
 if(next.kind==='reaction_rest'&&isReaction()&&(config.kind!=='reaction_rest'||next.token!==config.token))reactionSource=currentReaction();
 ...
 if(config.reduced&&!isReaction()&&!['flow_study','review_study'].includes(config.kind))done=false;
