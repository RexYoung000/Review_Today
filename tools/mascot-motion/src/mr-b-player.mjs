...
const isStudy=()=>config.kind==='flow_study'||config.kind==='review_study'||!!studyDuration[config.kind];
...
function duration(){return config.kind==='flow_study'?flow.duration():config.kind==='review_study'?reviewDuration:studyDuration[clip]??durations[clip]??rig.data.findAnimation(clip)?.duration??1;}
function apply(dt){
...
window.mrB={setState(next){
 ...
 if(config.reduced&&!['flow_study','review_study'].includes(config.kind))done=false;
