...
server.registerTool('preview_frame',{description:'Render an actual frame through official Spine runtime. Returns an image for AI visual inspection, not merely a URL.',inputSchema:{animation:animationSchema,time:z.number().min(0).max(9),size:z.number().int().min(240).max(960).default(640),dark:z.boolean().default(false)}},guard(async({time,size,dark,animation})=>{const p=await current();if(time>(animation==='recall'?p.recipe.duration:8))throw Error('Time exceeds animation duration');const png=await renderFrame(p.json,time,size,dark,animation);return {content:[{type:'image',mimeType:'image/png',data:png.toString('base64')}]};}));
