...
export function sample(rig,time,animation='recall'){
  rig.skeleton.setToSetupPose();
  rig.data.findAnimation(animation).apply(rig.skeleton,0,time,false,[],1,MixBlend.replace,MixDirection.mixIn);
...
export async function verifyRuntime(json,animation='recall'){
  const voices=animation==='recall'?Object.fromEntries(await Promise.all(['listening','speaking'].filter(name=>json.animations[name]).map(async name=>[name,await verifyRuntime(json,name)]))):undefined;
  const rig=await loadRig(json);const duration=rig.data.findAnimation(animation).duration;
  const steps=Math.ceil(duration*json.skeleton.fps)*2;
  let minArea=Infinity,maxCoordinate=0;
  for(let i=0;i<=steps;i++){
    const skeleton=sample(rig,duration*i/steps,animation),v=verticesOf(skeleton),tris=skeleton.findSlot('body').getAttachment().triangles;
  ...
  const first=verticesOf(sample(rig,0,animation)),last=verticesOf(sample(rig,duration,animation));
  ...
  return {parser:'official spine-core 4.2.120',sampledFrames:steps+1,minTriangleArea:minArea,maxCoordinate,endpointError,duration,...(voices?{voices}:{})};
}
