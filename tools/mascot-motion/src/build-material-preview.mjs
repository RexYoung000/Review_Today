...
html=html.replace('<canvas aria-hidden="true"></canvas>',`<div id="controls"><strong>黑白材质 · 动效对照</strong><label><input id="graphite" type="checkbox" checked>试作版</label><label><input id="dark" type="checkbox">深色</label><label><input id="reduced" type="checkbox">减少动态</label><label><input id="slow" type="checkbox">半速</label><select id="surface"><option value="recall">思考环绕</option><option value="voice">语音</option></select><select id="mode"><option value="idle">停止</option><option value="thinking">思考</option><option value="listening">聆听</option><option value="speaking">回答</option></select><input id="level" type="range" min="0" max="1" step=".01" value=".65" aria-label="模拟声量"><span id="status">模拟状态，不连接麦克风</span></div><canvas aria-hidden="true"></canvas>`);
