...
export async function renderer(json,size=640,dark=false,animation='recall',material='current'){
  ...
  const materials=createMaterials(images,createCanvas);renderer.materialImages=materials(material==='graphite'&&animation!=='recall'?'graphite-small':material,dark,animation==='recall');
  ...
  return time=>{
    if(animation!=='recall'){
      ...
      applyVoice(spine,rig,voice);ctx.setTransform(1,0,0,1,0,0);drawVoiceScene(ctx,renderer,rig,voice,size,size*.75,{dark,material});
      ctx.globalCompositeOperation='destination-over';ctx.fillStyle=material==='graphite'?(dark?'#131313':'#f6f6f6'):(dark?'#141918':'#f7f6f2');ctx.fillRect(0,0,size,size*.75);ctx.globalCompositeOperation='source-over';if(animation==='conversation'){ctx.fillStyle=material==='graphite'?(dark?'#b8b8b8':'#616161'):(dark?'#b4c8bb':'#476055');ctx.font='14px sans-serif';ctx.fillText({listening:'LISTENING / INWARD',thinking:'THINKING / BOUNCE',speaking:'SPEAKING / OUTWARD',idle:'STOP / SETTLE'}[conversationMode(time)],28,40);}return canvas.toBuffer('image/png');
    }
    sample(rig,time);ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle=material==='graphite'?(dark?'#131313':'#f6f6f6'):(dark?'#141918':'#f7f6f2');ctx.fillRect(0,0,canvas.width,canvas.height);ctx.translate(size/2,size*.75/2);ctx.scale(size/440,-size/440);renderer.draw(rig.skeleton);return canvas.toBuffer('image/png');};
}
export async function renderFrame(json,time,size=640,dark=false,animation='recall',material='current'){const frame=await renderer(json,size,dark,animation,material);return frame(time);}
export async function renderClip(json,name='recall',animation='recall',material='current'){
  ...
  const draw=await renderer(json,640,false,animation,material),child=spawn('ffmpeg',['-v','error','-f','image2pipe','-framerate',String(fps),'-i','pipe:0','-an','-c:v','libx264','-pix_fmt','yuv420p','-movflags','+faststart',destination],{stdio:['pipe','ignore','pipe']});
