...
- `npm test`：官方解析/加权形变实际生效、眼睛领先身体、权重与关键帧损坏检测、网格翻折拒绝；真实 stdio MCP 握手/工具清单/修改/绑定/图片返回/导出/失败后保留上一版；HTTP 同源/目录边界/错误请求检查，以及两个 MCP 进程同时修改时不丢失参数。7 项测试通过。
...
- 当前身体网格 193 顶点、336 三角形，11 根骨骼，135 个采样关键帧。24 fps 视频为 135 帧、实际 5.625 秒（动画标称时长 5.6 秒）。完整默认动画检查 271 个时间点，未发现翻折，首尾误差为 0。
