...
> 当前阶段：V11 只通过“角色不分离”验收；V12 真实 TTS 嘴型已完成实现、构建与原生冒烟，等待代表速度录屏和 Rex 最终体验验收
...
- GPT Image 生成一张无嘴完整语音母图，以及闭口、小开口、中开口、大开口和方牙笑局部位图；局部素材必须共享同一中心、比例、石墨线宽、奶油底色与陶土口腔色。每个张嘴轮廓必须完整封闭、上下均有自然弧度并保留透明安全边距，不得出现统一平顶、贴边或像被矩形裁断的横向切线。
- 真实 TTS 播放音量采用平滑后的包络驱动四档说话嘴型；静音与自然停顿必须回到闭口，不能随机持续张嘴。
- 方牙笑不按音量触发，只用于一句话结束后的短暂亲和反馈。
...
- 替换稿 `design-explorations/review-agent/mascot-mouth-sheet-v12.png` 由 GPT Image 生成。闭口、小开口、中开口、大开口和方牙笑均以独立完整轮廓呈现；三个开口嘴的上、下边缘都有连续弧线，不再使用 D 形平顶。
- `brand/split_mouth_sheet.swift` 只负责移除生成图的连通纸色背景、保留被轮廓包围的牙齿区域、切分和统一透明画布；它不重画嘴型。五张运行资源均为 135 × 95 px，并通过透明角与内容边界检查。
- `design-explorations/review-agent/mascot-mouth-animation-preview-v12.png` 是统一定位后的五状态静态检查板；`mascot-tts-speaking-wide-v12.png` 是最新 Debug 构建播放真实 TTS 时的原生高音量截图，完整椭圆嘴型无横向截断。
- 本机 TTS 检查结果为 13.92 秒，四档采样均被实际触发：闭口 191、小开口 135、中开口 853、大开口 21；Debug 构建通过。
- 当前只提升为“实现与冒烟通过”。仍缺一段覆盖连续说话、停顿、句末方牙笑、播放中打断和 Reduce Motion 的 10–15 秒原生录屏，因此尚不能宣布 V12 动态体验通过或正式 Spine 资产完成。

最终嘴型表生成提示词摘要：以已接受角色预览锁定细铅笔线、微小面部比例与陶土口腔色，以被拒绝嘴型表只锁定五状态顺序和中心；依次生成闭口微笑、完整小椭圆、完整中椭圆、完整大椭圆和两颗方牙微笑。所有开口必须是上下均有弧度的完整闭合手绘环，不得使用平顶、D 形、碗形、裁切边或共享水平线；背景保持单一暖白，后续只做机械去底和切分。
