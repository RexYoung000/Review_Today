...
> 当前阶段：V11 完整状态图 POC 已完成真实 Mac 冒烟；等待 Rex 视觉与动态节奏验收
...
- 使用 GPT Image 生成并选择的完整透明角色状态图；每张图必须是连体角色，不在运行时拆分头、身体和手臂。
- SpriteKit 只驱动完整角色位图的位移、旋转、缩放、透明度与轻微呼吸形变，不使用 SVG、Bezier 或 Canvas 重画角色。
- 使用独立开发窗口和本地演示状态，不读取或修改 SwiftData，不接入麦克风、Realtime、评分或 FSRS。
- 所有语音输入均为模拟控制；语音强度滑杆只验证完整说话角色的整体轻微响应，不再用独立拆件换取多档嘴型。
...
- 状态：待机／聆听／说话。
- 模拟语音强度：0–1 连续值，只在说话状态驱动完整角色的克制整体响应。
...
V11 已满足“角色不分离”的修正目标，但代表速度录屏、真实系统 Reduce Motion、最小窗口和 Rex 视觉验收仍未完成。未来若进入专业 Spine 拆件，全部部件在默认姿态归位时仍必须重组为已经验收的完整角色。
