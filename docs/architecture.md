...
2026-09-05 供应商修订：Rex 明确授权接入 DeepSeek 官方，契约见 Harness §20。独立 provider 选择隔离凭证与模型配置：Flash 承担路由/摘要与教学，Pro 承担风险判断；旧 Luna/Terra/Sol 映射保留为可恢复历史配置，不作为自动备用。Responses 系统指令使用受支持的 system/instructions，智能明确非思考、深入思考 high；复用现有投影、校验、预算和版本守卫。配置与能力探测不得仅凭模型列表报成功。当前接入状态/实测见验收 §0.13，下段 HTTP 403 为前批旧供应商记录。
