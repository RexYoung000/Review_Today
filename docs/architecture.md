...
当前以 Harness §17 为执行契约。下文 2026-09-04 锁屏/授权与工程完成描述为历史，不能代替当前状态。2026-09-05 第一批修复已实施，实际实现/测试及未完成限制见验收 §0.10；本日真实探测仍为上游 403。
...
- **当前探测与等待**：真实结构化能力探测默认 45 秒，模型步骤默认 90 秒；探测是后台服务状态，不产生 Session Event，也不作为当前 Run 阶段。服务与消息协议可用时，探测中／部分角色失败只更新能力诊断，不把连接状态降为不可用；全部角色不可用、服务不可达或协议不兼容才显示阻塞状态。角色不可用不静默替换，不阻塞不依赖该角色的步骤。本机服务 30 秒无健康响应则有限重启，最多四次；Debug 位于桌面目录，临时签名改变可能导致系统要求重新授权。权限由用户处理，不自动修改隐私设置。
