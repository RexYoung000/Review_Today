...
实时层使用阿里云北京 qwen3.8-omni-flash-realtime，由本地服务持有密钥代理 WebSocket，凭证沿用 providers/asr/.env，实时模型独立配置，文字模型不切换。原生 AVAudioEngine 负责采音／播放和打断；本机检测停顿，实时连接采用手动 audio commit，模型不能自主推进题目。播报音频在内存暂存，核对返回文本与程序授权文本一致后才播放；不一致保留文字通路。正式文字评价直接使用原始转写和固定规则；帮助／澄清／回答分别记录，语音模型不能擅改标准／日期或越过本地保存确认。

固定 swift-fsrs 源码版本 4fbaf20184d62f82a9f44f343337c61a2c5483e9（MIT），显式 v6 参数：默认21维权重、retention=.9、fuzz=false、learning/relearning=[10m]。完整调度状态及前后快照持久化；纠正从该次之前的状态重算，不叠加次数。迁移前用 SQLite Backup API 生成包含已提交 WAL 的一致备份，将备份切为 DELETE 日志模式后写入完成标记，以便单文件只读检查和恢复；源库模式不变。旧历史／到期保持，首次新复习重新初始化正式状态。

Knowledge 增加独立参与字段，新建默认资料，旧字段缺省视为原参与状态；与 lifecycle 分离。会话及尝试新字段均有兼容默认。技术失败／纯跳过不生成负面评分，关闭或暂停清理内存音频，永久保存文字和结果。服务记录声学转写回执、模型推理耗时／用量；实验记录端到端计时及本地保存结果，不把接口连通当作连续语音验收。

会话同步在派发前固定 revision、paused 与题目快照，避免暂停／恢复／下一题并发时拼接新版本和旧题目。纠正也递增同步版本，但不推进游标或增加复习次数。Mac 的本地提交是事实来源；服务 commit 仅为幂等回执，离线待发回执在恢复时重送。文本评价记录请求模型、供应商实际返回的模型标识、规则／提示词版本、原始输入及评价输出。

规则 V3 要求帮助披露字段：本题知识追问或澄清若透露答案线索，Mac 同样记录帮助；只有解释问法且未泄露知识的澄清不改变独立回忆条件。

验证与未覆盖项见 [语音复习证据](evidence/2026-09-23-review-voice/README.md)。

## A006/A007 增量契约（2026-09-17，已实现，待体验验收）
...
实现映射：`goal_ownership` 包含 `goal_id / owner_task_id / owner_session_id / version`；同一目标迁移到新任务段，旧任务段保持历史。SQLite 单次事务提交两侧投影；匹配前后的任务指纹校验用于阻止进度并发变化。Mac 的可选 `goalOwnershipJSON` 随事件与恢复游标持久化，默认旧任务 ID 为独立目标。`/v2/continuation/sources` 只检查来源检查点是否存在；缺失时由 Mac 恢复暂停检查点，不重放旧授权。来源恢复失败不阻止独立新问题。

输入路由没有有效当前目标而识别为 continue 时，必须核对真实续学记录；漏填字段不得落入创建新课程路径。恢复检查点按最高归属版本核对全部相关任务，不能靠恢复顺序复活旧任务。
...
## 指定材料与回复恢复（2026-09-26，已实现／定向验证）

指定材料入口统一提取有界 URL 集，逐源检查正文并保存来源/版本/失败原因；可用性判断独立于 HTTP 成功和证据核验。JD 节点接收实际材料和当前会话用途，保留待补材料状态；教学节点在材料不足时不创建学习计划。既有源缓存也必须重新通过正文检查。

结构恢复复用同一 Run、预算、模型与可见回答 ID，第二次生成不发布增量；完成校验后整段替换。原生客户端继续以 revision 和 chunk_seq 去重，并展示 recovering 状态；停止及迟到结果遵守原生命周期守卫。接口保持兼容，新状态和任务上下文字段均为增量。

本轮验证、真实回放错例及体验边界见 [指定材料修复记录](evidence/2026-09-26-material-routing/README.md)。

### 动态网页读取补充（2026-09-26，已实现／定向验证）

指定 URL 的 Exa/Tavily 提取失败后，显式启用的本机浏览器作为同一读取链的最后一项，不替换搜索或模型供应商。Python 服务使用固定版本 Playwright，在独立子进程启动临时浏览器；首选本机 Chrome，也支持已安装的 Playwright Chromium。运行时不下载浏览器，不接管用户 profile。凭证和私人对话不传给浏览器子进程。读取后销毁临时会话，取消、超时及旧运行版本不得继续回传或落盘。

浏览器经本机受限转发代理访问网页。代理解析并固定公网目标 IP，实际连接再次检查，重定向和子资源同样受约束；不放行 Fake-IP、私网或回环。系统 DNS 返回 Fake-IP 时，以固定 Cloudflare DoH 接口取得可校验的公网地址，只发送域名，不发送页面正文、原 URL 参数或凭据；内网解析不借此重试。浏览器取消隐式回环代理绕过，禁用非代理 UDP、Service Worker、WebSocket、下载及权限授权；只加载页面，不执行模型生成的点击／脚本。

读取链最多 60 秒，远端每服务 12 秒、本机浏览器最多 25 秒且一次；仍受累计网页 60 秒／10 次读取尝试及整轮预算约束。浏览器等待 DOM 及正文稳定，取得文字后继续既有正文／材料充分性检查；登录、验证、空壳和超时分别返回有限错误。结果包含实际最终 URL、截断状态与网络计数。事件只记录安全状态与统计，不记录浏览器请求头或脚本。

正常跳转新增的参数可留在浏览器临时会话；最终 URL 若触发凭证／隐私检查，只回传校验通过的稳定路径并标记参数已隐藏，原始用户 URL 单独保留。初始用户输入仍经过严格检查。可读正文的最终域名再次接受官方来源约束。普通页面成功与 BOSS 验证页失败均保留在实测报告中；BOSS 正文进入 JD 分析的目标此次未通过，不以基础浏览器可用替代该项验收。

真实回放发现英文证据摘录超过 240 字符后，一次格式恢复仍未改短。保留既有摘录长度和逐字出处校验；错误诊断补充来自 schema 的数字长度上限，恢复提示明确按字符缩短，不能静默截断证据或放宽材料充分性。
## 图片输入增量（2026-09-26，已实现／定向验证）

后续多图增量复用现有可空二进制字段，以有序附件数组编码，读取时兼容原单图编码，不新增数据库对象。Mac 拖放／多选／粘贴使用同一批次导入队列，按发起顺序交付，待处理数量也计入 8 张上限，会话变化使旧批次失效。服务的多图消息与逐图识读记录保留顺序；单图接口和旧记录继续兼容。

其他 App 拖放经 `NSDraggingInfo.draggingPasteboard` 接收，避免仅依赖 SwiftUI 转换后的图片提供者。文件承诺通过 `NSFilePromiseReceiver` 写入本批独立临时目录，在受协调的读取回调内生成附件，完成、失败或超时后清理本批临时文件。正文接收视图承载原有 SwiftUI 内容，普通点击和滚动继续交给内部视图；输入框使用同一接收入口。文件承诺数量未确定时为该批预留剩余图片容量，实际结果仍校验 8 张上限；草稿归属和取消沿用既有队列，不引入新持久化字段或系统权限。

普通文件 URL 也必须在拖放回调仍有效时读取为有大小上限的内存快照，不把来源缓存路径留到后台；后台只处理已取得的字节。文件读取使用有界流读取，不依赖额外的 URL 元数据查询或延迟内存映射。系统拒绝访问时区分权限失败和格式失败，仅记录读取阶段、类型、数量及错误域／码，不保存路径或原图日志；不通过扩大其他 App 数据权限解决导入失败。该生命周期修正须与真实微信拖入分别记录验证结果。

Mac 在 `AppSettings`／`AgentSession`／`AgentMessage` 的可空附件字段保存未发送／已发送图片，使用 SwiftData 外部二进制存储；只增加兼容字段，不改变既有对象身份。PNG/JPEG 原图经校验和去元数据后保存，正文继续单独存储。发送时通过现有消息接口携带图片，服务生成去字节的附件元数据与结构化识读，入口 Flash 使用 Responses `input_image`；其他节点接收有来源标记的文字派生材料。

服务原图仅保留在消息所属的本地状态中，向 Mac 同步的 checkpoint／事件增量排除图片字节，Mac 恢复后台状态时仅为尚未完成识读的消息按 ID 与摘要补齐；已完成识读的消息复用派生文本，不再次上传图片。恢复接口保留原 16 MB 文字限制，另允许累计最多 64 MiB 的图片字节（按 base64 上限核算），每张仍受 8 MiB 校验。Jev 与 Pro 不接收图片字节；不引入独立 OCR 网络服务。图片计入现有请求预算预估和供应商实际用量记录；后续依据整轮成本与真实样例质量决定是否增加本地 OCR 分流。删除与清空继承所属会话生命周期。

健康检查通过 `image_input_protocol` 声明图片能力，旧服务保留本地附件并提示重启，不静默丢图发送。图片识读结果同时进入教学准备、材料分析与公开查询的隐私检查；原始用户文字仍独立保留。`.NativeQA` 开发包缺少隔离参数时拒绝打开日常库。构建、契约、真实 Flash 与原生界面证据及阻塞见 [图片输入验证记录](evidence/2026-09-26-image-input/README.md)。
