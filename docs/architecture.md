...
四个主页面的现有身份／卸载逻辑保持。局部原生显示层用同纸面颜色的低透明度覆盖层完成 120 ms 轻淡入；覆盖层不参与点击、焦点或无障碍，不持有页面内容，不逐帧修改 SwiftUI 业务状态。快速切换、减少动态和窗口失活移除动画，模型透明度始终为零，避免过期完成回调留下遮挡。显示层动效不作为数据查询／布局提速证据。
