...
当前目标以 Harness §19、§18 及未被覆盖的 §17 为执行契约。§18 新能力待实施，下方 0.1 给出承载方式；既有代码/测试及未完成限制见验收 §0.10。旧锁屏/授权/403 都是相应测试的历史事实，本次文档回合不重新探测供应商。
...
- Mac 长期事实来源包括恢复快照；Python 清理需终态/ACK/无活跃引用。恢复缺失检查点不执行模型，不覆盖非缺失服务状态。当前只恢复覆盖已消费游标的完整快照，滞后快照不自动合成。旧磁盘增量迁移已在隔离 fixture 验证，不伪造历史。
...
当前代码基线 67e8760；§19 为文档冻结后的实施入口，不把目标字段写成已实现。Workspace 导航区分 Today/Agent 起始/Library/Inbox 与具体 Session ID；不依靠 nil 自动挑选最近会话。独立起始草稿在本机保存稳定 ID、正文、材料与偏好；首条 Session/Message/outbox 和草稿消耗在同一 SwiftData 事务保存，失败可重试同一 ID。
...
本轮还需补任意消费点增量快照合成、稳定步骤改名映射、来源刷新与混合引用分支。现有输入/流式/清单/热力图继续复用；新接口契约先失败测试后实现。原生验收和 provider 实测分别留证，未测不声明通过。协作规则调整只记录决定，不将全局个人配置纳入仓库。

### 0.1 本轮新增职责（目标，不是现有代码声明）
...
| 职责/拟增量数据 | 内容与写入边界 | 当前缺口 |
|---|---|---|
| 学习证据记录 | 稳定 ID、概念/范围、Session/消息/步骤出处、讲解/自述/独立评价类型、时间、提示使用、引用版本；作为可追溯记录自动维护，不写正式知识/FSRS | 现有步骤理解状态和复习记录分散，没有跨 Session 的统一证据检索 |
| 记忆使用策略 | Session 级是否允许跨会话引用及策略版本；排除后使派生摘要/检索缓存失效，重启和迟到结果不能重新启用 | 归档与记忆排除尚未分开；需安全默认与旧库迁移测试 |
| 关联检索上下文 | 相关记录/卡片 ID、版本、必要片段、关系用途、理解证据及时间；检索先限定允许范围，再排序 | `relevantKnowledgeSummaries` 当前只按字词匹配，传五条标题/学习目标字符串，没有证据状态 |
| 用户执行偏好 | 每 Session 的学习方式、思考强度；另存最近主动选择的强度，用于新 Session 初始化；有效值随 Run/快照固定 | 现有 modePreset 可复用，思考强度及迁移尚未实现 |
| 上下文预算 | 当前请求的输入计量/估计方法、有效预算、回答预留、上下文版本与摘要版本 | 当前 3k/16k 是目标预算，条数/字符裁切不等于精确 token 计量 |
| 内部执行策略 | 按角色与选定强度验证的主备名单、统一总时限/尝试预算、恢复原因与实际模型诊断 | 不存在已验证备用组合；本次不修改 provider 或启用新密钥 |

首版具体字段名和兼容 schema 随实现前的契约测试固化，不提前声称接口已存在。新增字段采用安全默认/可空值；没有原记录支持的理解状态不回填为已验证。旧字符串知识上下文保留兼容，新的类型化引用逐步替代；API 接收的引用必须验证所属用户、策略版本与对象状态，不能相信模型传来的任意 ID。
