...
当前以 Harness §17 为执行契约。下文 2026-09-04 锁屏/授权与工程完成描述为历史，不能代替当前状态。当前新增设计先冻结，实际实现/测试见验收 §0.10。
...
- Mac 长期事实来源包括恢复快照；Python 清理需终态/ACK/无活跃引用。恢复缺失检查点不执行模型，不覆盖非缺失服务状态。旧数据增量迁移且不伪造历史。
