...
## 标题与密度增量修订（2026-09-25）

标题和模块标签加粗，正文附加行距由 7pt 收至 4pt；段落／条目间距改为 12pt、区块间距 16pt、内边距 20pt。此项取代上方首轮排版数值。

只读核对日常库的 8 张旧卡，均没有独立 explanation；原步骤词来自 evidenceExcerpt。现在按内容来源显示「讲解／资料摘录／学习目标」，摘录回退明确提示暂无独立讲解；不重写资料，不修改评分或排期。

Debug 构建（`/tmp/knowledge-density-build.log`）及严格签名验证通过。重启日常 App，从知识库打开「RAG 的检索与生成阶段分别做什么」，原生截图与 AX 确认加粗标题、紧凑间距、摘录标识和原步骤正文；模块具有标题语义。没有运行模型、提交答案或改写知识内容。本次增量未重跑深色、窄窗、完整辅助技术；不将既有原型检查当作这次全场景通过。
