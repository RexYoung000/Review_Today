...
**当前入口与页面复修：**原生玻璃去掉深色覆盖与黑色外描边，增加悬停期间循环的反射及移动光点；右上角「小结预览」独立展示示例，支持重播。模拟考、最近学习、准备复习重新排版。当前截图、原速记录和检查边界见 [第六次修订](pages-and-glass-r6/README.md)。以下记录保留为历史。

**第五次预选与小结修订：**小结去掉横线并恢复部分完成时的 Spine 动作；玻璃入口补入扫光、亮闪与持续高光。46 项状态检查和原生增量证据见 [第五次修订](motion-feedback-r5/README.md)。⌘3 可重复演示玻璃预选，真实鼠标悬停手感仍待实测。

**第四次复习小结：**主卡与 Mr. B、独立结果卡、后续安排列表及固定操作区已按今天页风格重排，见 [小结页修订](summary-r4/README.md)。

**第三次材质试验：**「开始学习」「模拟考」两块入口采用系统原生玻璃，浅深色截图、原速交互录像及检查边界见 [玻璃入口试验](entry-glass-r3/README.md)。

**第二次今天页按视觉反馈修订：**复习主卡旁增加独立状态块，恢复三条可展开的最近学习和原有 26 周热力图。最新截图、三段原速录像与增量验证见 [今天页第二版](today-layout-r2/README.md)。下面今天页截图属于首轮，复习窗口证据仍适用。
...
今天页右上角「小结预览」直接打开独立的合成小结，可切换部分／全部完成并重播动作，不改变当前轮次。

原型菜单提供：⌘1 今天、⌘2 复习、⌘3 循环光效演示、⌘4 小结预览、⌘0 默认尺寸、⌘9 最小尺寸、⌘S 截图、⌘R 原速录制、⌘W 关闭窗口。答题时 Escape 暂停；帮助弹层可用 Tab、方向键、Return／Space，关闭后文字模式恢复输入焦点。
