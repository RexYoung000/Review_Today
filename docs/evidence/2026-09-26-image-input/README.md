...
### 其他 App 的原生拖放接收（2026-09-26，实现／来源实测待确认）

用户再次反馈上述版本仍失败，并说明来自其他 App（选项为微信／截图悬浮窗／其他 App，具体来源尚未确认）。本机 20:01:05 的系统日志记录：图片提供者读取 `public.jpeg` 失败，内部错误为 `NSCocoaErrorDomain/513`、`NSPOSIXErrorDomain/1`，发生在文件复制阶段。该证据说明此前只覆盖了模拟的表示切换，没有解决用户遇到的实际交接失败；不能将图片本身判定为损坏，也未修改系统权限。

本次将正文区从 SwiftUI 图片提供者桥接改为 AppKit 原生拖放接收，直接捕获当前拖放事件的 pasteboard，输入框复用相同入口。优先接收文件承诺，来源把图片生成到应用本批临时目录；普通图片优先使用来源提供的实际像素数据，文件 URL 由 AppKit 读取。每批仍有顺序、容量、会话归属和整批失败约束；30 秒未完成会回到原草稿并清理本批临时目录。新增仅含数量、路径类别与错误域／错误码的本机诊断，不记录图片内容或文件路径，不更改模型、网络或系统权限。

4 组当前源码原生契约通过：ImageInput、LearningInput、IndependentSession、InteractionFocus。新增验证原生正文／输入框路由、普通按钮命中、图片数据优先于不可读缓存 URL、真实 AppKit 文件承诺元数据、受控多文件承诺的逆序回调归位、失败／数量上限／超时及临时目录清理。承诺交付使用受控 receiver 验证；独立命令行直接调用真实文件承诺会被 AppKit 拒绝，因为它要求调用发生在真实拖放回调内，生产入口遵守该限制。正常签名构建与严格签名校验通过；摘要见 [原生拖放检查](native-drop-verification.txt)。

完整隔离 App 的会话正文、历史内容、图片预览和 Escape、逐张移除与文件选择添加、输入草稿、切换会话后原文字及图片恢复均已操作检查，未向日常库发送测试消息。该隔离包的后台端口本轮未就绪，未重复模型回放。原生独立拖放窗口已观察到鼠标按下和拖放开始事件，但自动化未完成目标投递，最终用 Escape 取消；仍不能记为真实文件承诺或用户原来源拖入通过。最终验收必须保留这一差距。

20:25 已将 `477a3a6` 的正常签名 Debug 包替换到日常启动路径，并重新打开原会话。旧 App、完整 SwiftData 数据目录和服务数据库备份在本机 `Review Today/Backups/before-native-image-drop-477a3a6-20260926-202518`。20:28 复核安装产物摘要与构建包一致，两份数据库完整性检查通过，业务模型和服务记录数不变；SwiftData 事务历史正常增长。日常后台图片协议 2、各模型角色 ready、Jev off；见 [原生拖放版运行状态](native-drop-daily-runtime.json)。未发送测试消息或改写用户草稿，用户原来源拖入仍待实测。


### 微信文件读取时机修正（2026-09-26，实际来源尚待验证）

21:24:53 的新失败已由 App 日志及系统 TCC 日志对应确认：来源为微信，只提供一个文件 URL，没有图片字节或文件承诺；pasteboard 已取得文件访问扩展，随后后台读取仍返回 `NSCocoaErrorDomain/257`。系统记录拒绝访问来源 App 的缓存数据。这里只保留错误域、时间和来源类别，不复制具体微信账号、文件名、缓存路径或图片内容。

检查发现普通 URL 仍被留到后台才读取。本次改为在原生拖放／粘贴回调内以有界读取保存图片字节，后台仅解码已取得的内存快照；不依赖 URL 元数据查询及延迟文件映射。文件快照仍执行 8 MiB 上限，不能使用剪贴板 TIFF 的较大中间字节额度。增加明确的权限错误分类及回调阶段诊断。没有扩大其他 App 数据权限，也没有确认或重置系统授权。

旧 `477a3a6` 模块的来源生命周期实验失败：捕获后删除来源临时文件，再完成导入即失败。该实验证明此前会继续依赖已失效的来源路径，不能单独证明修正后必定解决微信的 TCC 拒绝；两项验证保持区分。图片与输入两组当前源码原生契约通过；新增回归确认拖放／粘贴捕获后清除 pasteboard 并删除来源文件，图片仍完整导入，超过 8 MiB 仍拒绝，权限拒绝与文件不存在区分。首次测试因复用已关联的 NSPasteboardItem 违反 AppKit 测试夹具约束而中断，修正夹具后两组通过；应用模块与正常签名 Debug 构建使用相同源码，严格签名检查通过。没有重复模型回放。

21:35 已将 `e0db316` 的签名包替换到日常启动路径并打开原会话。原包和完整本地数据备份位于 `Review Today/Backups/before-drop-snapshot-e0db316-20260926-213500`；替换前后数据库完整性、业务记录数及安装产物摘要核对通过。后台协议 2、三个模型角色 ready、Jev off；见 [即时读取版运行状态](drop-snapshot-daily-runtime.json)。真实微信拖入结果尚待本轮继续核对，不因历史错误提示在重启后消失而记为成功。

### 微信访问被系统关闭（2026-09-26，已获授权开启／待拖入验证）

Rex 再次实测反馈「只能复制，不能拖拽」。21:43:57 即时读取版在原生拖放回调内失败，记录 `NSCocoaErrorDomain/513`；同一时刻系统明确记录 `kTCCServiceSystemPolicyAppDataDetailed` 拒绝 Review Today 读取来源 App 数据。原始拖放只有文件 URL 与 Qt 类型，没有 PNG/JPEG/TIFF 字节，也没有文件承诺。读取时机修正没有解决这个权限拒绝，因此微信拖入仍为失败，不再当作等待验收的成功候选。

通过系统设置真实界面展开「隐私与安全 → 文件与文件夹 → Review_Today.app」，确认「微信」开关为 off；只查看，没有切换或重置授权。已将系统设置停留在这一项，等待 Rex 决定是否允许。该系统权限的范围是微信本地数据，超出单张图片；它是系统授权，不是 Flash/OCR 模型能力或 JSON 格式问题。若允许开启，应随后用同一微信图片核对原生拖入与缩略图，成功前不能关闭验收。

Rex 回复「允许」后，已在该系统界面开启「微信」这一项，AX 状态确认由 off 变为 on。没有出现需重启的系统提示；已将原会话置前，未修改草稿或发送图片。21:54 核对尚无新的拖放日志，等待真实微信拖入后再记录结果；本轮无需替换 App 或重跑代码测试。
