...
2026-09-25 修复移出时的额外浅色闪烁：Spine 画布即时隐藏，底层静态图标却继承卡片的悬停动画，从 18% 墨色延迟恢复为完整墨色。现将这枚静态图标的透明度切换排除在卡片补间之外，退出时同帧复位；箭头和玻璃的原有过渡不变。原生对照页增加了可模拟预选退出的受控切换，在浅色、深色、减少动态及连续切换中检查静态图标；独立 WKWebView 的停播清空检查、原型和日常 App 构建、签名校验通过。已更新并打开本机日常 App，模拟考入口仍进入两种模式选择页；自由鼠标移出的主观手感仍由 Rex 在 App 中复核。
