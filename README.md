...
最新修复为开发服务启动保护：Debug 改用稳定开发签名，冷启动等待 90 秒，已就绪服务连续不可达 10 秒再恢复，并区分初始化阶段。编译、Mac 契约与无密钥启动冒烟通过；**证书私钥使用被钥匙串阻塞，待本机授权后完成签名构建与原生连续重启**。旧日常构建当前已恢复连接，原会话保留；详见 [验收 §0.18](docs/m1-acceptance.md#018-开发服务启动修复2026-09-06)。

最新增量为 **256K 活动上下文与即时悬停预览**：约 220K 按完整问答整理，目标约 144K；保留原文并取消原有 16 条 / 3,000 字符裁剪。圆环指针进入显示黑底白字「约已用 / 活动上限」。194 项服务测试、Mac 构建/契约、隔离真实长历史回问及原生发送通过；**日常 App 重启后 Python 路径读取阻塞复现，服务尚未恢复**。完整证据与未验证项见 [验收 §0.17](docs/m1-acceptance.md#017-256k-活动上下文与即时悬停预览2026-09-06)。
