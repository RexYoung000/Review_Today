...
Jev 交互测试：双击 [`tools/run-jev-app-test.command`](tools/run-jev-app-test.command)，构建并打开独立的「Review Today · Jev 测试」。进入 Agent，底部显示「Jev 测试已启用」后即可发送；输入会发送给 Jev 与现有模型。每次启动使用新的临时测试数据，退出此 App 即结束本次运行；日常 App 不受影响。首次使用可能出现 macOS 桌面文件夹访问提示，用于读取项目中的本地服务；处理系统提示后可在 Agent 顶部点「重试连接」。已有凭证从本机安全文件读取，不需再次输入 Key。实现与验证见 [本批记录](docs/evidence/2026-09-21-jev-app-test/README.md)。
...
| [Jev 与 DeepSeek 对照评测](docs/jev-deepseek-evaluation.md) | Harness 局部判断接入、隔离对照、真实调用证据与逐项采用建议 |
