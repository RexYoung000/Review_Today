...
此前增量为会话图标与上下文圆环：归档仅图标、记忆关闭标识可恢复、移除重复多选菜单。正常服务启动阻塞已在本次重启后恢复，独立原生真实请求验证了 4,721 / 1,000,000 token 与记忆开关跨启动保存；完整证据和剩余视觉验收见 [验收 §0.16](docs/m1-acceptance.md#016-会话图标记忆状态与上下文圆环2026-09-06)。分支已收口：issue 10–16 已合并并删除本地/远端分支，仅保留 main 与 issue-17；后者继续完整验收。结果见 [M1 分支核对](docs/m1-branch-audit.md)。
