...
当前 UI 增量为「Agent 界面交互精修」：图标侧栏、单层选择菜单、六项刷新快捷开始、保护预填与统一交互反馈。文档先提交推送，随后代码与受控回归完成；原生初轮已检查起始页、鼠标菜单与模板替换，后续被 Mac 锁屏阻塞，最终构建仍待复验。结果单列在 [验收 §0.14](docs/m1-acceptance.md#014-agent-交互精修2026-09-05)，不沿用旧测试冒充本轮通过；不更换 DeepSeek，不放行 M1/M2。
