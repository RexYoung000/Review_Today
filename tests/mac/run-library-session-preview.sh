...
folder = root / 'output/library-session-preview'
app = folder / 'ReviewTodayLibrarySessionPreview.app'
contents = app / 'Contents'
binary = contents / 'MacOS/ReviewTodayLibrarySessionPreview'
resources = contents / 'Resources'
binary.parent.mkdir(parents=True, exist_ok=True)
resources.mkdir(parents=True, exist_ok=True)
sources = sorted(str(p) for p in pathlib.Path('Review_Today').glob('*.swift') if p.name != 'Review_TodayApp.swift')
commands = [
    ['tests/mac/swift-with-fsrs.py', '-parse-as-library', '-D', 'DEBUG', '-swift-version', '5', '-default-isolation', 'MainActor', '-target', 'arm64-apple-macos26.5', *sources, 'tests/mac/LibrarySessionPreview.swift', '-o', str(binary)],
...
info = dict(CFBundleIdentifier='Rex.Review-Today.LibrarySessionPreview', CFBundleName='LibrarySessionPreview', CFBundleDisplayName='知识卡与会话原生验收', CFBundleExecutable=binary.name, CFBundlePackageType='APPL', LSMinimumSystemVersion='26.5', NSHighResolutionCapable=True, PreviewProjectRoot=str(root))
