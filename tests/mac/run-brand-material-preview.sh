...
set -euo pipefail
cd "$(dirname "$0")/../.."
preview_root="$PWD/output/brand-material-preview"
mkdir -p "$preview_root"
python3 - "$preview_root" <<'PY'
import pathlib, plistlib, shutil, subprocess, sys
folder = pathlib.Path(sys.argv[1])
app = folder / 'ReviewTodayBrandMaterialPreview.app'
contents = app / 'Contents'
binary = contents / 'MacOS' / 'ReviewTodayBrandMaterialPreview'
...
commands = [
    ['tests/mac/swift-with-fsrs.py', '-parse-as-library', '-D', 'DEBUG', '-swift-version', '5', '-default-isolation', 'MainActor',
     '-target', 'arm64-apple-macos26.5', *sources, 'tests/mac/BrandMaterialPreview.swift', '-o', str(binary)],
...
info = dict(CFBundleIdentifier='Rex.Review-Today.BrandMaterialPreview', CFBundleName='Review Today Brand Material Preview',
            CFBundleDisplayName='Review Today 黑白材质对照', CFBundleExecutable=binary.name,
            CFBundlePackageType='APPL', CFBundleVersion='1', CFBundleShortVersionString='1.0',
            LSMinimumSystemVersion='26.5', NSHighResolutionCapable=True, PreviewProjectRoot=str(pathlib.Path.cwd()))
