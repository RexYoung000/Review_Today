...
@main struct BrandMaterialContractTests {
    ...
    @MainActor static func main() throws {
        ...
        for dark in [false,true] {
            let p=Runway.palette(dark:dark)
            precondition(p.monochrome && p == RunwayPalette.brandMonochrome(dark: dark))
        ...
        print("PASS: daily palette is monochrome; legacy comparison remains; memory-only trial rejects sending and keeps draft")
    }
}
