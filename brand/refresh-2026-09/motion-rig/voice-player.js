...
import {createVoiceState,advanceVoice,applyVoice,drawVoiceScene,contactMoving,contactGeometry} from './voice-scene.mjs';
...
let state;
const labels={listening:'正在聆听 · 波动向内汇聚',thinking:'正在思考',speaking:'正在回答 · 波动向外传出',idle:'已停止'};
$('reduce').checked=reduced;
function draw(canvas,compact=false){const box=canvas.getBoundingClientRect(),d=Math.min(devicePixelRatio||1,2),w=Math.round(box.width*d),h=Math.round(box.height*d);if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;}const ctx=canvas.getContext('2d');ctx.setTransform(d,0,0,d,0,0);const renderer=new MeshRenderer(ctx);renderer.triangleRendering=true;drawVoiceScene(ctx,renderer,rig,state,box.width,box.height,{compact,reduced,debug:$('debug').checked,dark:document.body.classList.contains('dark')});}
function status(){
  const c=state?.contact,announcement=labels[mode]+(reduced?' · 减少动态效果':'');
  if($('modeStatus').textContent!==announcement)$('modeStatus').textContent=announcement;
  let label=labels[mode];
  if(c&&mode==='speaking'&&c.answerClock>=0)label=c.answerClock<.12&&!c.replyLaunched?'句首 · 柔软蓄力':c.replyPending?(c.velocity<80?'提前下瞟 · 准备落下':'句首 · 轻轻提起'):c.answerWaveAge<.35?'实际接触 · 推出声波':'句中 · 轻微起伏';
  if(c&&mode==='thinking')label={charge:'柔软承托 · 准备起跳',flight:'腾空 · 看向落点',land:'圆头接触 · 波浪回弹'}[c.phase]??'正在思考';
  if(c&&mode==='listening')label=c.height>.65?'轻托离面 · 舒展身体':c.impact>.15?'柔软落回 · 接住波动':labels.listening;
  if(c&&mode==='idle'&&contactMoving(c))label='已停止 · 落稳并收住余波';
  label+=reduced?' · 减少动态效果':Number($('volume').value)===0&&['listening','speaking'].includes(mode)?' · 等待声音':'';
  $('phase').textContent=label;$('hero').setAttribute('aria-label',label+'，语音角色与波浪预览');
}
...
$('debug').addEventListener('change',wake);
...
(async()=>{try{const atlas=new spine.TextureAtlas(await(await fetch('data/images/mascot.atlas')).text());await Promise.all(atlas.pages.map(async page=>{const image=new Image();image.src='data/images/'+page.name;await image.decode();page.setTexture(new spine.CanvasTexture(image));}));
  ...
  const data=new spine.SkeletonJson(new spine.AtlasAttachmentLoader(atlas)).readSkeletonData(source);rig={data,skeleton:new spine.Skeleton(data)};state=createVoiceState(contactGeometry(spine,rig));downloadURL=URL.createObjectURL(new Blob([JSON.stringify(source)],{type:'application/json'}));$('download').href=downloadURL;
  for(const b of document.querySelectorAll('[data-mode]'))b.disabled=false;$('health').textContent='圆头接触 · 局部回弹 · 面部跟随';wake();
