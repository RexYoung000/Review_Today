...
export function materialPixel(name,r,g,b,active=false,palette=materialPalette(),x=.5,y=.5,lightBody=false){
 const luminance=.2126*r+.7152*g+.0722*b;
 if(name==='body.png'){const v=Math.round(Math.max(0,Math.min(255,37-24*clamp((y-.18)/.62)-4*Math.pow((x-.5)/.42,2)+(luminance-70)*.35)));const tone=lightBody?Math.round(Math.max(0,Math.min(255,210+(v-25)*1.1))):v;return [tone,tone,tone];}
 if(name==='eye.png')return lightBody?[35,35,35]:[247,247,247];
 if(name==='pupil.png')return lightBody?[235,235,235]:[35,35,35];
...
export function createMaterials(images,makeCanvas){
 ...
 return function materialImages(style,dark,active=false,override){
  if(!['graphite','graphite-small'].includes(style))return undefined;
  const lightBody=style==='graphite-small';
  const palette=materialPalette(dark,override),key=JSON.stringify([palette,lightBody]);
  if(!cached.has(key)){
   ...
   for(const [name,image] of images){
    const render=active=>{
     ...
     for(let i=0;i<data.length;i+=4){const rgb=materialPixel(name,data[i],data[i+1],data[i+2],active,palette,(i/4%canvas.width)/canvas.width,Math.floor(i/4/canvas.width)/canvas.height,lightBody);data[i]=rgb[0];data[i+1]=rgb[1];data[i+2]=rgb[2];}
