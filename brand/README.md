...
文档独立推送后才制作。交付实际原生导出 PNG，工具支持时附 SVG/透明素材、可复现参数和工程；大图及16/32/64/128px、浅深背景检查，不能用截图或放大替代母版。来源/使用条件/限制和Rex选择保存于 `oneworks/`，只增加必要记录。Rex选定并原生验收前不改AppIcon；V8及动态POC保留。下文模型位图-only规则仅适用于V8，不限制用户已批准的OneWorks新路线。
